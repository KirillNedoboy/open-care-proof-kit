(() => {
  ...
  const send = async (question) => { emptyState?.remove(); const message = document.createElement("article"); message.className = "message message-user"; const bubble = document.createElement("div"); bubble.className = "user-bubble"; bubble.textContent = question; message.append(bubble); stream.append(message); textarea.value = ""; sendButton.disabled = true; status.textContent = "Checking vault context and sources…"; try { const headers = { "content-type": "application/json" }; if (endpoint === "/api/chat") headers["X-OpenCare-CSRF"] = csrfToken(); const response = await fetch(endpoint, { method: "POST", credentials: "same-origin", headers, body: JSON.stringify({ question }) }); const payload = await response.json(); if (!response.ok) throw new Error(payload.detail || "OpenCare could not process this request."); addAnswer(payload); } catch (error) { const messageText = error instanceof Error ? error.message : "OpenCare could not process this request."; addAnswer({ answer: messageText, citations: [], unknowns: [], doctor_questions: [], boundary_notices: ["No provider output was displayed."], status: "validation_failed" }); } finally { sendButton.disabled = false; status.textContent = ""; textarea.focus(); stream.scrollTop = stream.scrollHeight; } };
