"""OpenCare-specific trust adapter: live Family Access authorization.

This module is OpenCare-specific and MAY depend on Family Access / Product Core.
The generic trust package (``app.agent_trust``) must never import this module;
generic trust code depends only on the ``AuthorizationAdapter`` protocol in
``app.agent_trust.authorization`` and never grants access itself.
"""
