...
def test_evidence_assessment_fails_closed_for_absent_no_call_build_and_orientation() -> None:
    ...
    imported = parse_consumer_genotypes(
        synthetic_input(
            "rsDemoPGX\t10\t1001\tAG", "rsDemoNoCall\t11\t1101\t--", "rsDemoAmbiguous\t12\t1201\tAT"
        ),
        genome_build=GenomeBuild.GRCH37,
    ...
    results = {
        result.evidence_id: result for result in assess_evidence(imported.observations, pack)
    }
    ...
    assert all(
        results[key].finding is None
        for key in ("ev-absent", "ev-no-call", "ev-build-mismatch", "ev-orientation")
    )
...
def test_research_packet_is_selective_deterministic_and_contains_no_raw_bytes() -> None:
    ...
    finding = next(
        result.finding for result in assess_evidence(imported.observations, pack) if result.finding
    )
...
def test_ibs_zero_common_fails_closed() -> None:
    left = parse_consumer_genotypes(
        synthetic_input("rsLeft\t1\t101\tAA"),
        genome_build=GenomeBuild.GRCH37,
        selected_rsids={"rsLeft"},
    ).observations
    right = parse_consumer_genotypes(
        synthetic_input("rsRight\t2\t202\tGG"),
        genome_build=GenomeBuild.GRCH37,
        selected_rsids={"rsRight"},
    ).observations
...
def test_output_validation_rejects_invented_citations_and_requires_devil_advocate_sections() -> (
    None
):
...
def test_speculative_model_background_must_be_explicitly_labelled() -> None:
    ...
    with pytest.raises(ValueError, match="model-background"):
        validate_research_output(
            valid_output(model_background=True, epistemic_status=EpistemicStatus.SUPPORTED), packet
        )
...
def test_pgx_intersection_uses_exact_normalized_confirmed_medications_only() -> None:
    ...
    assert [(match.medication_name, match.evidence_id) for match in matches] == [
        ("demomed", "ev-pgx")
    ]
...
def test_research_packet_rejects_raw_source_fields() -> None:
    packet = _packet_for_validation()
    with pytest.raises(ValueError, match="raw"):
        build_research_packet(
            person_id=packet.person_id,
            mode=packet.mode,
            observations=packet.selected_observations.values(),
            evidence_entries=packet.selected_evidence.values(),
            findings=packet.selected_findings.values(),
            selected_observation_ids=set(packet.selected_observations),
            selected_evidence_ids=set(packet.selected_evidence),
            selected_finding_ids=set(packet.selected_findings),
            selected_health_records={
                "raw-source": {"kind": "document", "source_content": "rsX 1 1 AA"}
            },
        )
