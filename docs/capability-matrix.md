...
> P1 branch updates: the rows below reflect the `codex/p1-evidence-grounded-ingest`
> branch (implementation complete, not yet integrated to `main`). The historical
> Phase-2 dates above are preserved; P1 changes are marked inline.
...
| Document upload | `OUT_OF_SCOPE` | No upload route or handler in `app/main.py`; reviewer UI explicitly rejects this scope (unchanged by P1). |
...
| Extraction | `PARTIAL` | Explicit plain-text source registration exists; no document extraction, OCR, or model extraction (unchanged by P1 — P1 adds none). |
| Review inbox | `PARTIAL` | P1 branch: medication + condition + lab candidates at `/workspace` (medication review inbox plus condition/lab pending surfaces); broader review remains unsupported. |
| Canonical confirmed records | `PARTIAL` | P1 branch: all three fact families (medication/condition/lab) confirm transactionally into `canonical_records` with typed detail; still no other fact families. |
| Timeline | `PARTIAL` | P1 branch: medication/condition/lab confirmation + correction events; demo read model remains separate. |
...
| Conditions | `IMPLEMENTED` (P1 branch) | Source-backed recorded-condition lifecycle in `app/product_core/` (candidate → provenance → review → canonical condition record); explicitly a record, not a diagnosis. |
| Labs | `IMPLEMENTED` (P1 branch) | Source-preserving lab lifecycle in `app/product_core/` (typed text values, source flags as-provided); no unit conversion or interpretation. |
...
| Visit preparation | `IMPLEMENTED` | Workspace supports persistent Visits, Questions, Visit-scoped immutable Brief revisions, confirmed-evidence selection, preparation notes, restore history, and audited Markdown export; P1 branch extends evidence selection to condition/lab records (content schema v2; v1 revisions readable). |
| Guarded chat | `IMPLEMENTED` | Live chat requires an Actor session, accessible active Person, and `chat.use`; the server builds isolated Product Core context from the question-only request (P1 branch adds confirmed condition/lab evidence items). |
...
| Evaluations | `IMPLEMENTED` | `evals/runner.py`, `evals/cases/`, `evals/trust_metrics.py`, `tests/test_evals_runner.py`, `tests/test_trust_metrics.py`; P1 branch adds the deterministic offline reviewer `python -m evals.p1_review` (`tests/test_p1_reviewer.py`). |
...
| Backup and export | `PARTIAL` | Person export (P1 branch: format v3 with condition/lab entities) and offline schema backup/verify/preflight/recover (P1 branch: schema v7) preserve durable access state but no sessions. No import, merge, encryption, or populated-target recovery exists. |
