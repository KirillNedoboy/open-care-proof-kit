...
| Guarded chat | `PARTIAL` | The existing G2 consent contract and live route are present, but the normal application wiring is being closed by the R3 live-chat integration; no clinical correctness claim. |
| External LLM provider | `PARTIAL` | The legacy Responses adapter and configuration gates exist, but provider-specific G2 execution and external live smoke evidence are not yet current. |
