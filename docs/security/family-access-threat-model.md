...
| Ownerless new Person | Person creation requires `confirm_owner_assignment: true` and inserts the Person, owner assignment, and audit atomically. | No Person is created on missing confirmation, failed authorization, constraint failure, or audit failure. | Transaction-failure cases leave no orphan Person, owner assignment, or success audit. |
...
| Person enumeration via responses | Person routes use the same `404` for missing, inactive, and unauthorized targets. `403` is reserved for known non-Person controls, CSRF, owner-invitation confirmation, and `confirm_owner_assignment: true` validation after authorization. Lists reveal no hidden names, IDs, memberships, counts, or installation totals. | No response differentiates target existence or reveals hidden list data. | Response equivalence tests compare body, headers, and status for missing versus unauthorized Person targets and list-redaction tests. |
...
| Actor session survives disaster recovery | Session store is separate from Product Core SQLite and is never read, copied, verified, preflighted, or restored by backup/recovery. It has no persistent volume and startup rejects overlap with Product DB/source paths. Offline `backup`, `verify`, `preflight`, and `recover` are installation-operator workflows requiring no Actor session, assignment, or Person impersonation. | Recovered installation has no valid pre-recovery session. | Backup layout and recovery tests prove no session-store access, actor/session-independence, and required reauthentication afterward. |
...
| Bootstrap or deactivation bypass | `/api/family-access/v1` has only the approved bootstrap, session, self, password, selection, actor-list, invitation preview/register/accept, and Actor-deactivation routes. Bootstrap requires zero Actors and the private-production outer `/access` gate. Deactivation atomically revokes admin/access state, appends consent revocations, clears own link, and then invalidates sessions. | Bootstrap/deactivation fails without partial durable state; inactive Actor is denied even if session cleanup fails. | Route-contract, actor-list privacy, zero-Actor, bootstrap-atomicity, invitation-route, last-owner/last-admin, deactivation, and cleanup-failure tests. |
