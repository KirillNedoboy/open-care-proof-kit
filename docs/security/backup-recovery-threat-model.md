...
This is the proposed Phase 1F threat model. Future portable exports and
installation backups contain sensitive health data in plaintext. They require
operator-controlled destinations and local handling. Phase 1F does not add
encryption, signing, cloud storage, scheduled jobs, sharing or public backup and
...
| Incomplete source store | Enumerate source IDs from the database snapshot; verify each copied payload size/hash. | Backup is incomplete without marker; recovery rejects it. |
